        ...
        <div class="scoreboard">
            <div class="header">
                <div v-if="logos && logos.length">
                    <img class="logo" v-for="(logo, index) in logos" :key="index" :src="require(`@/assets/img/logos/${logo}`)" :alt="`${logo.split('.')[0]}-Logo`" />
                </div>
            ...
            <div class="participant-list">
                <div class="participant" v-for="(participant, index) in sortedParticipants" :key="participant.name">
                    ...
                    <div class="info-container" v-html="notesContent(index, sortedParticipants)"></div>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    computed: {
        ...
        status() {
            const highestMatchShotCount = Math.max(...this.fetchedData.map(p => p.matchShotCount));
            if (highestMatchShotCount === Math.max(...this.fetchedData.map(p => p.matchSize))) {
